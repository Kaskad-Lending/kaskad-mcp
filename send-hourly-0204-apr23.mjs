...
HF: ∞ ✅  |  Collateral: $247,644  |  Debt: $0  |  Available: $164,658
APY snapshot: KSKD 64.72%  |  IGRA 56.16%  |  WBTC borrow 2.42%
Net yield: $38,782/yr  |  Net APY: 15.66%  |  Monthly: $3,232/mo
KSKD Eligibility: Supplier ✅  |  Borrower ✅  |  Epoch 14
Action: No KSKD rewards to claim. Debt=0 with borrower eligibility active — borrow WBTC (~$39.8K at 2.42%) to capture borrower-share emissions; HF stays ~5.2, well above 1.8.
\`\`\``;
  ...
  if (!connected && data.type === 'res' && !data.error) {
    ...
    ws.send(JSON.stringify({
      ...
      params: {
        ...
        idempotencyKey: 'keal-hourly-0204-20260423',
