...
const msg = `\`\`\`
[03:06] Kaskad Agent — Hourly Check
HF: ∞ ✅  |  Collateral: $247,173  |  Debt: $0  |  Available: $164,271
APY snapshot: KSKD 64.43%  |  IGRA 55.97%  |  WBTC borrow 2.43%
Net yield: $38,756/yr  |  Net APY: 15.68%  |  Monthly: $3,230/mo
KSKD Eligibility: Supplier ✅  |  Borrower ✅  |  Epoch 12
Action: Borrow WBTC (~$30K at 2.43%) to capture borrower-share emissions — HF stays >> 1.8. KSKD 100% util (unavailable). No KSKD rewards to claim.
\`\`\``;
  ...
  if (!connected && data.type === 'res' && !data.error) {
    ...
    ws.send(JSON.stringify({
      ...
      params: {
        ...
        idempotencyKey: 'keal-hourly-0306-20260424',
