...
const msg = `\`\`\`
[15:01] Kaskad Agent — Hourly Check
HF: ∞ ✅  |  Collateral: $247,590  |  Debt: $0  |  Available: $164,598
APY snapshot: KSKD 64.70%  |  IGRA 56.20%  |  WBTC borrow 2.42%
Net yield: $38,797/yr  |  Net APY: 15.67%  |  Monthly: $3,233/mo
KSKD Eligibility: Supplier ✅  |  Borrower ✅  |  Epoch 14
Action: Borrow WBTC (~$40K at 2.42%) to activate borrower-share emissions — HF stays ~5.2, well above 1.8. No KSKD rewards to claim.
\`\`\``;
  ...
  if (!connected && data.type === 'res' && !data.error) {
    ...
    ws.send(JSON.stringify({
      ...
      params: {
        ...
        idempotencyKey: 'keal-hourly-1501-20260423',
