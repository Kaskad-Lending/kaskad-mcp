...
const msg = `\`\`\`
[02:01] Kaskad Agent — Hourly Check
HF: ∞ ✅  |  Collateral: $247,134  |  Debt: $0  |  Available: $164,369
APY snapshot: KSKD 63.81%  |  IGRA 54.47%  |  WBTC borrow 2.42%
Net yield: $38,281/yr  |  Net APY: 15.49%  |  Monthly: $3,190/mo
KSKD Eligibility: Supplier ✅  |  Borrower ✅  |  Epoch 12
Action: No KSKD rewards to claim. Debt=0 while borrower-eligible — borrow WBTC (~$38.5K min at 2.42%) to capture borrower-share emissions, HF stays >> 1.8.
\`\`\``;
  ...
  if (!connected && data.type === 'res' && !data.error) {
    ...
    ws.send(JSON.stringify({
      ...
      params: {
        ...
        idempotencyKey: 'keal-hourly-0201-20260422',
