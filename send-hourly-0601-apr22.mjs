...
HF: ∞ ✅  |  Collateral: $248,619  |  Debt: $0  |  Available: $165,456
APY snapshot: KSKD 64.55%  |  IGRA 54.87%  |  WBTC borrow 2.42%
Net yield: $38,461/yr  |  Net APY: 15.47%  |  Monthly: $3,205/mo
KSKD Eligibility: Supplier ✅  |  Borrower ✅  |  Epoch 14
Action: No KSKD rewards to claim. Debt=0 with borrower eligibility active — borrow WBTC (~$39K min at 2.42%) to capture borrower-share emissions, HF stays >> 1.8.
\`\`\``;
  ...
  if (!connected && data.type === 'res' && !data.error) {
    ...
    ws.send(JSON.stringify({
      ...
      params: {
        ...
        idempotencyKey: 'keal-hourly-0601-20260422',
